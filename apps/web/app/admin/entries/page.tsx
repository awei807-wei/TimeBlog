...
export default function EntriesAdmin() {
  ...
  async function integrationMutation<T>(path: string, body?: unknown): Promise<T> {
    const csrf = await fetch(`${API}/auth/session`, { credentials: 'include' }).then(r => r.json()).then(v => v.csrfToken || '');
    const response = await fetch(`${API}${path}`, { method: body === undefined ? 'POST' : 'PATCH', credentials: 'include', headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': csrf }, body: body === undefined ? undefined : JSON.stringify(body) });
  ...
  async function saveImageHost(patch: { enabled: boolean; endpoint: string; token: { action: 'keep' | 'replace' | 'clear'; value?: string } }) {
    setBusy(true); setError(''); setMessage('保存图床配置中…');
    try { const value = await integrationMutation<ExternalImageHostConfig>('/admin/integrations/external_image_host', patch); setImageHost(value); setMessage('图床配置已安全入库；协议验证前仍使用本地媒体'); }
    catch (cause) { setError(cause instanceof Error ? cause.message : '图床配置保存失败'); }
    finally { setBusy(false); }
  }

  async function testImageHost() {
    setBusy(true); setError(''); setMessage('执行无副作用可达性探测…');
    try { const result = await integrationMutation<{ status: string; message: string }>('/admin/integrations/external_image_host/test'); setImageHost(current => current ? { ...current, status: result.status, statusMessage: result.message } : current); setMessage(result.message); }
    catch (cause) { setError(cause instanceof Error ? cause.message : '图床探测失败'); }
    finally { setBusy(false); }
  }

  async function saveNASBackup(value: Omit<NASBackupConfig, 'applyStatus' | 'status' | 'statusMessage' | 'lastTestedAt' | 'updatedAt'>) {
    setBusy(true); setError(''); setMessage('保存 NAS pull 策略中…');
    try { const saved = await integrationMutation<NASBackupConfig>('/admin/integrations/nas_backup', value); setNASBackup(saved); setMessage('NAS pull 策略已入库，等待导出到 NAS 环境文件'); }
...
function SettingsPanel({ values, runtime, imageHost, nasBackup, onSave, onSaveImageHost, onTestImageHost, onSaveNAS }: {
  ...
  onSaveImageHost: (value: { enabled: boolean; endpoint: string; token: { action: 'keep' | 'replace' | 'clear'; value?: string } }) => void;
  onTestImageHost: () => void;
  onSaveNAS: (value: Omit<NASBackupConfig, 'applyStatus' | 'status' | 'statusMessage' | 'lastTestedAt' | 'updatedAt'>) => void;
}) {
  const [draft, setDraft] = useState(values);
  const [imageDraft, setImageDraft] = useState({ enabled: imageHost?.enabled || false, endpoint: imageHost?.endpoint || 'https://image.cainiao.me/api/uploads', token: '', clearToken: false });
  ...
  return <section className="settings-panel" aria-labelledby="settings-title">
    ...
    <div className="settings-layout">
      ...
      <Card className="settings-card"><CardHeader className="settings-card-heading"><div><span className="eyebrow">RUNTIME HEALTH</span><CardTitle>运行状态</CardTitle></div><UploadCloud aria-hidden="true"/></CardHeader><CardContent>{runtime ? <div className="settings-status-list"><div><span>媒体存储</span><strong>本地规范原件 <em>local_private</em></strong>{status(runtime.media.writable)}</div><div><span>上传能力</span><strong>{runtime.media.imageUploadEnabled ? '图片与附件可上传' : '上传已禁用'}</strong>{status(runtime.media.imageUploadEnabled && runtime.media.nonImageUploadEnabled)}</div><div><span>上传上限</span><strong>{Math.round(runtime.media.maxUploadBytes / 1024 / 1024)} MB</strong></div><div><span>外部发布</span><strong>API 协议待验证 · 当前不发送文件</strong><span className="settings-status">继续使用本地媒体</span></div></div> : <div className="empty">运行状态载入中…</div>}</CardContent></Card>
      <Card className="settings-card integration-card"><CardHeader className="settings-card-heading"><div><span className="eyebrow">CUSTOM PUBLIC</span><CardTitle>外部图床</CardTitle></div><CloudCog aria-hidden="true"/></CardHeader><CardContent className="settings-form"><p className="settings-help">Token 至少需要“写入图片”权限；未来启用远端清理时还需要“删除图片”。当前缺少官方 API 文档，因此保存与无副作用探测可用，但不会上传或删除任何文件。</p><label>上传 API<input type="url" value={imageDraft.endpoint} onChange={event => setImageDraft(current => ({ ...current, endpoint: event.target.value }))}/></label><label>Token<input type="password" autoComplete="new-password" value={imageDraft.token} placeholder={imageHost?.tokenConfigured ? '********（留空保持不变）' : '输入只写 Token'} onChange={event => setImageDraft(current => ({ ...current, token: event.target.value, clearToken: false }))}/></label><div className="integration-options"><label><input type="checkbox" checked={imageDraft.enabled} onChange={event => setImageDraft(current => ({ ...current, enabled: event.target.checked }))}/> 保存为启用意图</label><label><input type="checkbox" checked={imageDraft.clearToken} onChange={event => setImageDraft(current => ({ ...current, clearToken: event.target.checked, token: '' }))}/> 清除已保存 Token</label></div><div className="integration-state"><strong>{imageHost?.tokenConfigured ? 'Token 已加密保存' : 'Token 未配置'}</strong><span>{imageHost?.statusMessage || '等待保存配置'}</span></div></CardContent><CardFooter className="integration-actions"><button type="button" className="primary" onClick={() => onSaveImageHost({ enabled: imageDraft.enabled, endpoint: imageDraft.endpoint, token: imageDraft.clearToken ? { action: 'clear' } : imageDraft.token ? { action: 'replace', value: imageDraft.token } : { action: 'keep' } })}>保存图床配置</button><button type="button" className="secondary" disabled={!imageHost} onClick={onTestImageHost}>仅测试可达性</button></CardFooter></Card>
