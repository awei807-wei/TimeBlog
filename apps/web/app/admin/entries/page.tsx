...
export default function EntriesAdmin() {
  ...
  async function testImageHost(endpoint: string) {
    return integrationMutation<{ status: string; message: string }>('/admin/integrations/external_image_host/test', 'POST', { endpoint });
  }
...
function SettingsPanel({ values, runtime, imageHost, nasBackup, onSave, onSaveImageHost, onTestImageHost, onSaveNAS }: {
  ...
  onSaveImageHost: (value: { enabled: boolean; endpoint: string; token: { action: 'keep' | 'replace' | 'clear'; value?: string } }) => Promise<void>;
  onTestImageHost: (endpoint: string) => Promise<{ status: string; message: string }>;
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      setDraft(values);
      if (imageHost) setImageDraft({ enabled: imageHost.enabled, endpoint: imageHost.endpoint, token: '', clearToken: false });
  ...
  async function handleImageHostProbe(event: React.MouseEvent<HTMLButtonElement>) {
    ...
    await runEndpointProbe({ event, endpoint: imageDraft.endpoint, probe: onTestImageHost, onState: setProbeState });
  }
