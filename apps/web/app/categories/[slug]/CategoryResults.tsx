...
export default function CategoryResults({ slug, initialEntries, initialCursor }: { slug: string; initialEntries: PublicEntry[]; initialCursor?: string }) {
  ...
  return <section className="timeline">{entries.length ? entries.map((entry, index) => <article className="entry" key={entry.id || index}><div className="entry-meta">{entry.journalDate}<span className="tag">{entry.kind === 'article' ? '文章' : '随记'}</span></div>{entry.title && <h2>{entry.slug ? <Link href={`/article/${entry.slug}`}>{entry.title}</Link> : entry.title}</h2>}<p>{entry.summary || entry.markdown}</p></article>) : <div className="empty">这个分类还没有公开内容。</div>}{error && <div className="error-panel" role="alert">{error}</div>}{cursor && <button type="button" className="secondary load-more" onClick={() => void loadMore()} disabled={loading}>{loading ? '加载中…' : '加载更多'}</button>}</section>;
}
