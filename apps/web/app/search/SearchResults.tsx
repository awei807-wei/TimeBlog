...
function ResultCard({ entry }: { entry: PublicEntry }) {
  ...
  return <article className="entry"><div className="entry-meta"><span>{entry.journalDate}</span><span className="tag">{entry.kind === 'article' ? '文章' : '随记'}</span></div>{entry.title && <h2>{entry.slug ? <Link href={`/article/${entry.slug}`}>{entry.title}</Link> : entry.title}</h2>}<p>{entry.summary || entry.markdown}</p><div className="entry-links">{entry.tags?.map(t => <Link href={`/tag/${encodeURIComponent(t)}`} className="tag" key={t}>#{t}</Link>)}</div></article>;
}
