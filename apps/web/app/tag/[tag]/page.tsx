...
export default async function TagPage({ params }: { params: Promise<{ tag: string }> }) {
  ...
  return <main id="main-content" className="shell"><header className="topbar"><Link href="/" className="brand">个人时间线</Link><nav className="nav"><Link href="/calendar">日历</Link><Link href="/search">搜索</Link></nav></header><div className="eyebrow">TAG</div><h1>#{data.tag}</h1><TagResults tag={tag} initialEntries={data.entries} initialCursor={data.nextCursor}/></main>;
}
