...
export default function TagResults({ tag, initialEntries, initialCursor }: { tag: string; initialEntries: PublicEntry[]; initialCursor?: string }) {
  ...
  async function loadMore() {
    ...
    setError('');
    try {
      const data = await getTag(tag, cursor);
  ...
  return <section className="public-result-list" aria-live="polite">
    {entries.length ? entries.map((entry, index) => <PublicEntryCard entry={entry} showDate key={entry.id || index} />) : <div className="public-empty">这个标签还没有公开内容。</div>}
