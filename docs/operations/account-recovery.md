...
3. 优先在 Google Authenticator 中选择“扫描二维码”，扫描恢复成功页显示的 TOTP 二维码。二维码由当前浏览器在本地生成，不会把 TOTP Secret 发送给第三方。
4. 仅当二维码无法扫描时，才在 Google Authenticator 中选择“输入设置密钥”，并只复制页面独立显示的“手动设置密钥”：它是仅含 `A-Z` 和 `2-7` 的 32 字符 Base32 文本。将类型选为“基于时间”，账户名可记为“菜鸟手记 / owner”。
5. 页面显示的完整 `otpauth://` URI 只供支持该协议的本地应用直接导入。它包含 `:/?=&%` 等 URI 字符，不能粘贴到 Google Authenticator 的手动密钥框；也不得提交给外部网站、联网转换工具或第三方二维码服务，否则会泄露 TOTP Secret。
6. 在离开恢复结果页前，确认新恢复密钥和 TOTP 手动设置密钥均已保存到受控密码库，不要将其复制到普通笔记、工单或聊天记录。
7. 重新登录，完成新密码与 Google Authenticator 当前 TOTP 验证码的验证，确认旧会话均已撤销。
8. 确认新恢复材料可用后，按备份保留策略处理运行目录；不要把其中任何文件复制到 Git、普通工单或聊天记录。
