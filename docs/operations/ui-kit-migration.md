...
- Novel 正文使用轻量 sticky 工具带直接提供图片、撤销重做与高频格式，窄屏可横向滚动且图片入口保持首位；选区气泡菜单和 `/` 命令仍提供就近快捷操作，触控目标不小于 44px。
- 图片面板同时支持本地上传和 HTTPS 图床外链，并允许填写替代文字；本地上传继续使用 `media://` 与异步发布链，外链不复制到本地媒体卷。
