...
- 新快照成功并写入 `_SUCCESS` 后按数量淘汰：本地默认保留 7 份、WebDAV 默认保留 90 份，支持 dry-run。只有固定文件集合完整、严格时间戳且有远端成功标记的受管快照可以删除。
...
- 未知目录、失败临时目录、无 `_SUCCESS`、缺件或含额外内容的目录不会被自动淘汰；这会优先保护数据，但也意味着异常目录仍需人工审查容量。
